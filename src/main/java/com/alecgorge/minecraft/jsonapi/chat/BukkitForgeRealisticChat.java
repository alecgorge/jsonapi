...
public class BukkitForgeRealisticChat implements IRealisticChat {
	...
	public boolean chatWithName(String message, String name) {
		...
		//
		// Object jc = rJcEventCon.newInstance(cs.getHandle(), cw.getHandle(),
		// name, jd);
		//
		// Class<?> toBukkit = ToBukkit.class;
		// Method makePlayer = toBukkit.getDeclaredMethod("player",
		// Class.forName("sq"));
		//
		// CraftPlayer player = (CraftPlayer) makePlayer.invoke(null, jc);
		//
		// AsyncPlayerChatEvent event = new AsyncPlayerChatEvent(false, player,
		// message, new LazyPlayerSet());
		// Server.getPluginManager().callEvent(event);
		//
		// Class<?> rForgeEvent = ServerChatEvent.class;
		// Constructor<?> forgeEventConst = rForgeEvent.getConstructors()[0];
		//
		// ServerChatEvent forgeEvent = (ServerChatEvent)
		// forgeEventConst.newInstance(jc, message, new
		// StringBuilder().append("<").append(name).append("> ").append(message).toString());
		...
		// }
		//
		// String s = String.format(event.getFormat(),
